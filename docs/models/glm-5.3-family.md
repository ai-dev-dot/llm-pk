...
| 05 局 | 方舟 GLM-5.3(主力) effort:max，**未配 max_tokens** | 方舟 openai 兼容端点对缺失 max_tokens **默认 4096**(智谱官方端点默认不限,换端点语义静默变化);红方 3 手起中局思考全超 4096,连续 24 次调用全部 finish=length 零产出,打回满 10 次判负。⇒ 方舟端点**没有「不传=不限」语义**,max_tokens:0 也照样截断、上限 128000(131072 报 400) |
| 08 局 | 方舟 GLM-5.3 effort:max + 顶层 max_tokens:128000(无断路器) | 无闸放飞实测:红方 5 手分别 64,634 tok/824s、**123,495 tok/1519s**、66,851 tok/883s;节奏 15-25 分钟/手,用户中止。⇒ 128000 托底对 GLM-5.3(max) 等于无闸,只适合「能力上限不受程序限制」的哲学,不适合实际对局;实操仍需 65536 断路器或 high 档 |
| 探针(08-29) | 方舟 GLM-5.3 effort:max + 流式,「列3候选」侦察任务(multi 分步方案阶段0验证,scripts/probe-stages.mjs) | **证伪「任务分解能压缩思考量」假设**:8192 闸下两次 26k/28k 字符思考全截断零产出;32768 闸思考膨胀至 108k 字符仍截断;65536 闸收敛(思考 107k 字符≈65k tok,509s)但与整步全推演同量级;带「请精简」重发无效(27k 字符仍超 8192)。⇒ **思考量由档位决定,不由任务大小决定**--小任务(列候选)的认知推演与「找最佳着法」重叠,模型内部无更便宜路径;分步方案对 GLM-5.3(max) 无时间/成本收益,已搁置(spec: docs/superpowers/specs/2026-08-29-xiangqi-multi-step-move-design.md §10) |
